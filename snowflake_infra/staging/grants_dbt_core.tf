...
resource "snowflake_grant_privileges_to_account_role" "dbt_dev_schemas" {
  for_each = {
...
resource "snowflake_grant_privileges_to_account_role" "dbt_dev_bronze_objects" {
...
resource "snowflake_grant_privileges_to_account_role" "dbt_dev_bronze_future_objects" {
  for_each = toset(["TABLES", "EXTERNAL TABLES"])
